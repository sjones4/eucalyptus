/*
 * Copyright 2020 AppScale Systems, Inc
 *
 * Use of this source code is governed by a BSD-2-Clause
 * license that can be found in the LICENSE file or at
 * https://opensource.org/licenses/BSD-2-Clause
 */
package com.eucalyptus.cloudwatchlogs.common.msgs;

import edu.ucsb.eucalyptus.msgs.EucalyptusData;
import javax.annotation.Nonnull;
import com.eucalyptus.cloudwatchlogs.common.CloudwatchlogsMessageValidation.FieldRange;
import com.eucalyptus.cloudwatchlogs.common.CloudwatchlogsMessageValidation.FieldRegex;
import com.eucalyptus.cloudwatchlogs.common.CloudwatchlogsMessageValidation.FieldRegexValue;


public class QueryCompileErrorLocation extends EucalyptusData {

  private Integer endCharOffset;
  private Integer startCharOffset;

  @com.fasterxml.jackson.annotation.JsonProperty("endCharOffset")
  public Integer getEndCharOffset( ) {
    return endCharOffset;
  }

  public void setEndCharOffset( final Integer endCharOffset ) {
    this.endCharOffset = endCharOffset;
  }

  @com.fasterxml.jackson.annotation.JsonProperty("startCharOffset")
  public Integer getStartCharOffset( ) {
    return startCharOffset;
  }

  public void setStartCharOffset( final Integer startCharOffset ) {
    this.startCharOffset = startCharOffset;
  }

}
